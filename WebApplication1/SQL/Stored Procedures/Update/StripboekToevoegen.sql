...
IF (SELECT Id FROM gebruiker WHERE gebruiker_id = Id)
THEN
BEGIN
...
WHERE reeks_naam NOT IN (
  SELECT reeks.reeks_naam FROM reeks
);
...
WHERE cat_naam NOT IN (
  SELECT categorie.cat_naam FROM categorie
);
...
WHERE uitgever_naam NOT IN (
    SELECT uitgever.uitgever_naam FROM uitgever
);

#Uitgave
INSERT INTO Uitgave(naam, hoogte, beschrijving,cat_id, reeks_id)
SELECT @naam, @hoogte, @beschrijving, cat_id, reeks_id
FROM categorie, reeks
WHERE cat_naam = categorie.cat_naam AND reeks_naam = reeks.reeks_naam;
    
#Versie
INSERT INTO Versie(Versie_id,afbeelding_url,isbn, datum, druk, prijs, uitgever_id, uitgave_id)  
SELECT null, @afbeelding_url,@isbn, @datum, @druk, @prijs,uitgever_id, uitgave_id
FROM uitgever, uitgave
WHERE (uitgever_naam = uitgever.uitgever_naam AND naam = uitgave.naam);
